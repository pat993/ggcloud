...
class M_player extends CI_Model
{
    ...
    {
        $this->db->select('assigned.id as assign_id, ip, port, port_audio, custom_name, access_token, end_date_kompensasi');
        $this->db->where($where);
        $this->db->where('end_date_kompensasi>=CURRENT_TIMESTAMP()');
